...
class SFConnectionIT : SFBaseTest
{
	...
	public void TestBasicConnection()
	{
		using (var conn = new SFConnection())
		{
			...
			Assert.AreEqual("", ((SFConnection)conn).DataSource);

			var serverVersion = ((SFConnection)conn).ServerVersion;
	...
	public void TestLoginTimeout()
	{
		if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
		{
			using (var conn = new MockSnowflakeDbConnection())
			{
				...
				{
					Assert.AreEqual(SFError.RequestTimeout, e.SnowflakeError);
				}
				stopwatch.Stop();
	...
	public void TestConnectionFailFast()
	{
		using (var conn = new SFConnection())
		{
			...
			{
				Assert.AreEqual(SFError.InternalError, e.SnowflakeError);
			}
	...
	public void TestInvalidConnectionString()
	{
		...
		var expectedErrorCode = new[] { SFError.MissingConnectionProperty, SFError.InvalidConnectionString, SFError.InvalidConnectionString };
	...
	public void TestUnknownAuthenticator()
	{
		...
		foreach (var wrongAuthenticator in wrongAuthenticators)
		{
			try
			{
				var conn = new SFConnection() { ConnectionString = "scheme=http;host=test;port=8080;user=test;password=test;account=test;authenticator=" + wrongAuthenticator };
				conn.Open();
				Assert.Fail("Authentication of {0} should fail", wrongAuthenticator);
			}
			catch (SFException e)
			{
				Assert.AreEqual(SFError.UnknownAuthenticator, e.SnowflakeError);
			}
		}
	}
