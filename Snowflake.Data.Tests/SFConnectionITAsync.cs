...
class SFConnectionITAsync : SFBaseTestAsync
{
	...
	public void TestAsyncLoginTimeout()
	{
		if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
		{
			using (var conn = new MockSnowflakeDbConnection())
			{
				...
				{
					Assert.AreEqual(SnowflakeError.RequestTimeout, ((SnowflakeException)e.InnerException).SnowflakeError);
				}
				stopwatch.Stop();
	...
	public void TestAsyncConnectionFailFast()
	{
		using (var conn = new SnowflakeConnection())
		{
			...
			{
				Assert.AreEqual(SnowflakeError.InternalError, ((SnowflakeException)e.InnerException).SnowflakeError);
			}
